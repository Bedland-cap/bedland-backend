...
public class ManagerControllerTest {
    ...
    void shouldReturnAllManagersWhenGettingAllMembers() throws Exception {
        ...
        //then
        mockMvc.perform(get("/manager")).andDo(log()).andExpect(status().isOk()).andExpect(jsonPath("$").isArray())
    ...
    void shouldEmptyListWhenGettingAllManagersAndThereAreNone() throws Exception {
        ...
        //when
        //then
        mockMvc.perform(get("/manager")).andDo(log())
    ...
    void shouldReturnManagerWhenGettingManagerById() throws Exception {
        ...
        //then
        mockMvc.perform(get("/manager/{id}", sampleId)).andDo(log()).andExpect(status().isOk())
    ...
    void shouldReturnNoRecordWhenWhenGettingManagerByIdAndIDIsNotInDB() throws Exception {
        ...
        //when
        //then
        mockMvc.perform(get("/manager/{id}", sampleId)).andDo(log()).andExpect(status().isOk()).andExpect(jsonPath("$[0]").doesNotExist());
    }
    ...
    void shouldCreateManagerWhenCreatingManager() throws Exception {
        ...
        String requestedBody = """
                ...
                """.formatted(creationRequest.getLogin(), creationRequest.getPassword(), creationRequest.getName(), creationRequest.getLastName(), creationRequest.getEmail(), creationRequest.getPhoneNumber());
        //when
        when(managerServiceMock.create(creationRequest)).thenReturn(creationRequest);
        //then
        mockMvc.perform(post("/manager").contentType(MediaType.APPLICATION_JSON).content(requestedBody)).andDo(log()).andExpect(status().isCreated())
    ...
    void shouldUpdateManagerWhenUpdatingManager() throws Exception {
        ...
        //then
        mockMvc.perform(patch("/manager").contentType(MediaType.APPLICATION_JSON).content(requestedBody)).andDo(log()).andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(creationRequest.getId()))
                .andExpect(jsonPath("$.login").value(creationRequest.getLogin()))
                .andExpect(jsonPath("$.password").value(creationRequest.getPassword()))
                .andExpect(jsonPath("$.name").value(creationRequest.getName()))
    ...
    void shouldDeleteMemberWhenDeletingMember() throws Exception {
        ...
        //when
        //then
        mockMvc.perform(MockMvcRequestBuilders.delete("/manager/{id}", sampleId)).andDo(log()).andExpect(status().isNoContent());
    }
}
